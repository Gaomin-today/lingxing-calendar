    ...
    var body: some View {
        ...
        Card {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("以选中日 \((try? MilestoneEngine().dateText(selectedDate)) ?? "—") 为基准 · 北京时间")
                ...
                if result.entries.isEmpty {
                    ...
                    ForEach(Array(result.entries.prefix(5))) { entry in
                        MilestoneOccurrenceRow(entry: entry, onSelectDate: onSelectDate)
                    }
                    if result.entries.count > 5 {
                        Button("查看全部 \(result.entries.count) 项") { managing = true }.buttonStyle(.plain)
                ...
                Text("仅在首页显示，不发送系统通知。").font(.system(size: 10)).foregroundStyle(Theme.secondary)
...
struct MilestoneEditor: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 19) {
            ...
            Text(MilestoneEngine.timeZoneNote).font(.system(size: 11)).foregroundStyle(Theme.secondary).lineSpacing(4)
                ...
                VStack(alignment: .leading, spacing: 17) {
                    ...
                    Card {
                        VStack(alignment: .leading, spacing: 13) {
                            ...
                            Text("每份档案独立选择，默认关闭；选择生日不会切换当前命盘。只在首页显示，不发送系统通知。")
                                .font(.system(size: 11)).foregroundStyle(Theme.secondary).lineSpacing(4)
